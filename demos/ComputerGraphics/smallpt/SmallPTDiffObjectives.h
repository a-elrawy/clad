...
inline double diff_objective_pixel(int pixel_x, int pixel_y, int samps,
  ...
  unsigned short Xi[3] = {0, 0,
                          (unsigned short)(pixel_y * pixel_y * pixel_y)};
  ...
  for (int sy = 0; sy < 2; ++sy) {
    for (int sx = 0; sx < 2; ++sx) {
      ...
      for (int s = 0; s < samps; ++s) {
        ...
        double u =
            ((sx + .5 + jitter_x) / 2 + pixel_x) / (double)w - .5;
        double v =
            ((sy + .5 + jitter_y) / 2 + pixel_y) / (double)h - .5;
        Vec cx_term = cxv * u;
...
inline double diff_objective_demo_aa(double cam_x, double cam_y, double cam_z) {
  return diff_objective_pixel(256, 192, 1, cam_x, cam_y, cam_z,
...
inline double diff_objective_patch_mean(double cam_x, double cam_y, double cam_z,
  ...
  for (int py = y0; py < y0 + patch_h; ++py) {
    for (int px = x0; px < x0 + patch_w; ++px) {
      double pix = diff_objective_pixel_aa_samps(px, py, cam_x, cam_y, cam_z,
                                                 samps);
      sum = sum + pix;
    }
  }
