...
namespace clad {
namespace custom_derivatives {
void sample_erand48_pullback(unsigned short* Xi, double d_out,
                             unsigned short* d_Xi) {
}
} // namespace custom_derivatives
} // namespace clad
...
static inline double vec_avg(const Vec& v) {
...
// Floor patch where mirror-reflected path-traced radiance is non-zero at defaults.
...
static inline bool diff_intersect(double rox, double roy, double roz,
  ...
  for (int i = kNumSpheres; i--;) {
    ...
    if (i == kLightSphereId)
      spy = light_y;
    double d = diff_sphere_intersect(
        kDiffScene[i].rad, spx, spy, kDiffScene[i].pos.z, rox, roy, roz, rdx,
        rdy, rdz);
    if (d > 0 && d < t) {
      t = d;
      id = i;
