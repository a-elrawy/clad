...
static inline bool smallpt_essentially_equal(double a, double b, double rel = 5e-2) {
...
using smallpt_patch_fd_fn = double (*)(double, double, double, int, int, int, int,
                                       int);

static inline double smallpt_central_fd_patch(smallpt_patch_fd_fn f, double x,
                                              double y, double z, int x0, int y0,
                                              int pw, int ph, int samps,
...
static inline int smallpt_run_patch_grad_check(int patch_w, int patch_h,
  ...
  grad.execute(cam_x, cam_y, cam_z, x0, y0, patch_w, patch_h, samps, &g_x,
               &g_y, &g_z);
...
static inline double smallpt_central_fd_mirror_patch(smallpt_mirror_patch_fd_fn f,
                                                    double mirror_x, int x0,
                                                    int y0, int patch_w,
                                                    int patch_h, int samps,
                                                    double h = 1e-4) {
...
static inline int smallpt_run_mirror_patch_grad_check() {
  ...
  double radiance = diff_objective_patch_mirror_x(mirror_x, x0, y0, patch_w,
                                                  patch_h, samps);
  if (radiance < 0.01) {
  ...
  bool pass = smallpt_essentially_equal(
      g_x, smallpt_central_fd_mirror_patch(diff_objective_patch_mirror_x,
                                          mirror_x, x0, y0, patch_w, patch_h,
                                          samps));
